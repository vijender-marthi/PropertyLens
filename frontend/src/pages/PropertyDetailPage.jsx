...
function normalizeAnnualExpense(row, year = CURRENT_YEAR) {
  return {
    ...
    advertising: numberOrEmpty(row?.advertising),
    auto_travel: numberOrEmpty(row?.auto_travel),
    cleaning_maintenance: numberOrEmpty(row?.cleaning_maintenance),
    commissions: numberOrEmpty(row?.commissions),
    legal_professional: numberOrEmpty(row?.legal_professional),
...
function annualExpensePayload(row, year = CURRENT_YEAR) {
  return {
    ...
    advertising: inputNumber(row?.advertising),
    auto_travel: inputNumber(row?.auto_travel),
    cleaning_maintenance: inputNumber(row?.cleaning_maintenance),
    commissions: inputNumber(row?.commissions),
    legal_professional: inputNumber(row?.legal_professional),
...
const EDITABLE_EXPENSE_FIELDS = [
  { key: 'advertising', label: 'Advertising', decor: 'other_expenses' },
  { key: 'auto_travel', label: 'Auto and travel', decor: 'other_expenses' },
  { key: 'cleaning_maintenance', label: 'Cleaning and maintenance', decor: 'repairs' },
  { key: 'commissions', label: 'Commissions', decor: 'management_fees' },
  { key: 'insurance', label: 'Insurance', decor: 'insurance' },
  { key: 'legal_professional', label: 'Legal and professional fees', decor: 'other_expenses' },
  ...
  { key: 'home_warranty', label: 'Home Warranty', decor: 'other_expenses' },
...
function ValueWaterfallStoryChart({ waterfall, onJump, showTitle = true }) {
  ...
  const barWidth = 48
  // Spread the bars to fill the chart width (no dead space on the right) for the
  // usual handful of nodes, with a sensible minimum for longer stories.
  ...
  return (
    <div className="min-w-0 py-1">
      ...
      <div className="mx-auto mt-4 max-w-[760px] overflow-x-auto">
        <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="mx-auto w-full min-w-[560px]" role="img" aria-label={waterfall.title}>
